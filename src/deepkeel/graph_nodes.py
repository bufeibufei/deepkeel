...
def _forced_tool_fallback_arguments(
    *,
    ...
    for name in set(spec.required_args).union(
        ...
        sources = (
            configured_sources if isinstance(configured_sources, list)
            else [configured_sources] if configured_sources else []
        )
        value = next((
            resolved
            for source in sources
            if (resolved := _forced_tool_argument_source(
                str(source or ""), state=state, context_bundle=context_bundle,
            )) not in (None, "")
        ), None)
        if value in (None, ""):
            configured_paths = context_bindings.get(name)
            paths = (
                configured_paths if isinstance(configured_paths, list)
                else [configured_paths] if configured_paths else []
            )
            value = next((
                resolved
                for path in paths
                if (resolved := _mapping_path_value(
                    context_bundle, str(path or ""),
                )) not in (None, "")
            ), None)
