...
def test_release_contract_accepts_matching_semantic_version() -> None:
    ...
    assert verify_release_version(repo_root, "v3.1.0") == "3.1.0"


def test_release_contract_rejects_mismatched_tag() -> None:
    repo_root = Path(__file__).resolve().parents[1]

    with pytest.raises(ValueError, match="release tag must be v3.1.0"):
        verify_release_version(repo_root, "v3.0.2")
